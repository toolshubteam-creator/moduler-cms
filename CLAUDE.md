...
- `TenantDbContext` modul entity'leri `OnModelCreating`'de `IHasEntities.RegisterEntities` cagrilarak runtime'da kayit edilir; cekirdek `DbSet` deklare edilmez. **Istisna:** Platform-tenant infrastructure tablolari (orn. `Audit_Entries` — Faz-3.1) tenant DB'de mapping yapilir ama `DbSet` exposed edilmez; runtime'da `ctx.Set<T>()` ile erisilir. Bu tablolar modul-bagimsiz cekirdek davranisin parcasidir (audit, soft-delete vs.) ve `Audit_` gibi platform prefix'i kullanir.
